@echo off
echo 4_testAutorizaciones.bat
echo Ejecuta el estresador para probar el servicio de Autorizaciones (WSSolicitaAutorizacion)
echo.
echo Uso:
echo   4_testAutorizaciones.bat [NumClientes] [NumSolicitudesPorCliente] [HOSTNAME]
echo.
echo   - NumClientes: Numero de clientes concurrentes (default: 20)
echo   - NumSolicitudesPorCliente: Numero de autorizaciones por cliente (default: 10)
echo   - HOSTNAME: Host del servidor (default: localhost)
echo.
@echo on

REM Configuración del POJO
set POJO=parcial_02_pojo_1.Parcial_02_POJO_Sol
set POJOJAR=2_POJO_Autorizaciones.jar
