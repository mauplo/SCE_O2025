@echo off
echo 4_testEnvios.bat
echo Ejecuta el estresador para probar el servicio de Envios (WSEnvios)
echo.
echo Uso:
echo   4_testEnvios.bat [NumClientes] [NumSolicitudesPorCliente] [HOSTNAME]
echo.
echo   - NumClientes: Numero de clientes concurrentes (default: 20)
echo   - NumSolicitudesPorCliente: Numero de envios por cliente (default: 10)
echo   - HOSTNAME: Host del servidor (default: localhost)
echo.
@echo on

REM Configuración del POJO
set POJO=pojo_envios.POJO_Envios
set POJOJAR=3_POJO_Envios.jar
